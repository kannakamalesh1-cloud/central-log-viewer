...
import { Server, Activity, Lock, Loader2, Plus, X, Eye, EyeOff, CheckCircle2, AlertCircle, KeyRound, ChevronRight, Trash2, Settings, RotateCw } from 'lucide-react';
...
export default function Sidebar({ userRole, onSelect }: SidebarProps) {
  ...
  const openPanel = () => { setForm(defaultForm); setEditingId(null); setSaveStatus('idle'); setSaveError(''); setShowAddPanel(true); setShowUserPanel(false); };
  const closePanel = () => { setShowAddPanel(false); setEditingId(null); setSaveStatus('idle'); };
      ...
      <div className={`w-80 h-full flex flex-col bg-white/5 dark:bg-black/30 backdrop-blur-xl border-r border-white/10 dark:border-zinc-800 p-6 overflow-y-auto transition-transform duration-300 ${showAddPanel ? '-translate-x-full' : 'translate-x-0'} absolute inset-0`}>
        ...
        {selectedType && (
          <div className="flex flex-col gap-2">
            ...
            {loadingSources ? (
              <div className="flex items-center justify-center gap-2 text-purple-400 text-sm py-8 border border-white/5 rounded-xl bg-black/20">
                <Loader2 className="w-5 h-5 animate-spin" />
                <span>Scanning for {selectedType} logs...</span>
              </div>
            ) : sourceError ? (
              <div className="flex flex-col items-center gap-2 text-red-400 text-xs py-6 px-4 border border-red-500/10 rounded-xl bg-red-500/5 text-center">
                <AlertCircle className="w-5 h-5 mb-1" />
                <span className="font-semibold uppercase tracking-tight">Discovery Failed</span>
                <span className="text-zinc-400 leading-relaxed font-mono">{sourceError}</span>
                <button 
                  onClick={() => setSelectedType(null)}
                  className="mt-2 text-purple-400 hover:text-purple-300 underline underline-offset-4"
                >
                  Try again
                </button>
              </div>
              ...
              <div className="flex flex-col gap-2 max-h-[300px] overflow-y-auto pr-1">
                {logSources.length === 0 && (
                  <span className="text-zinc-500 text-sm">No items found</span>
                )}
                {logSources.map((source, idx) => (
                  ...
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <span className="truncate max-w-[180px]">{source.identifier}</span>
                    </div>
      ...
      <div className={`w-80 h-full absolute inset-0 flex flex-col bg-black/60 backdrop-blur-2xl border-r border-white/10 transition-transform duration-300 ${showAddPanel ? 'translate-x-0' : 'translate-x-full'}`}>
        ...
        <form onSubmit={handleAddServer} className="flex flex-col gap-4 p-6 overflow-y-auto flex-1">
          ...
          <button
            ...
            className="mt-2 w-full bg-purple-600 hover:bg-purple-500 disabled:bg-purple-900/50 disabled:cursor-not-allowed text-white font-semibold rounded-xl py-3 transition-all flex items-center justify-center gap-2 text-sm"
          >
            {saving ? (
